...
CAMERA_URL = "http://192.168.1.181:8081/video"      # your IP Webcam address
SMS_GATEWAY_URL = "http://192.168.1.181:8080/send-sms"  # your SMS Gateway address
    ...
    while True:
        ...
        if now - last_check >= CHECK_INTERVAL:
            ...
            try:
                ...
                if predictions:
                    # Log everything detected, for visibility
                    for p in predictions:
                        label = p.get("animal_name", "Unknown")
                        confidence = p.get("confidence", 0)
                        print("[DETECTED]", label, "(confidence=%.2f)" % confidence)

                    # Only act on the single strongest detection across both models
                    ...
                    if now - last_alert_time >= ALERT_COOLDOWN:
                        ...
                        print("(Cooldown active, %ds remaining — top match was %s at %.2f)"
                              % (remaining, top.get("animal_name", "Unknown"), top.get("confidence", 0)))
                else:
                    print("No detection.")
            except Exception as e:
